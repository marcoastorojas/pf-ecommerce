...
const registerUser = async (req = request, res = response) => {
  ...
  await Promise.allSettled([Person.create({ name, userId: newUser.uid }), Status.create({ userId: newUser.uid })]);
...
const googleAuth = async (req = request, res = response) => {
  try {
    ...
    if (!user) {
      ...
      await Promise.allSettled([Person.create({ name, lastname, userId: user.uid }), await Status.create({ userId: user.uid })]);
    }
...
const loginUser = async (req = request, res = response) => {
  ...
  try {
    ...
    const user = await User.findOne({
      where: {
        [Op.or]: [{ email: email_user }, { username: email_user }],
      },
    });

    if (!user) {
      return res.status(401).json({ errors: { email_user: `no existe un usuario con el email o username ${email_user}` } });
    }
